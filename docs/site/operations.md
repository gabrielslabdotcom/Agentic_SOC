...
| Unit | Role |
|------|------|
| `agentic-soc-autonomy` | Poll Wazuh → score → open cases → Discord notify → Cursor cloud |
| `agentic-soc-discord-bot` | Gateway bot for analyst-outcome buttons (outbound WS only; no new inbound port) |
...
**Do not overwrite Pop `.env`.** Do not recreate the Discord webhook unless it was revoked. Gateway bot setup (token/channel) is in SETUP_GUIDE §12.1 — outbound websocket only.
...
rsync -av --exclude '.venv' --exclude 'data/' --exclude '.env' \
  /path/to/Agentic_SOC/ <ssh-alias>:$AGENTIC_SOC_HOME/
```

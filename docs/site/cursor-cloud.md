...
The cloud agent must **not** Approve/Reject or execute containment. Public cloud VMs **cannot** reach `<SIEM_HOST>`. That is why Pop persists the note locally.
...
If the Cursor GitHub App cannot see the repo, autonomy logs a validation error and (by default) retries **no-repo** (case JSON only). Grant the App access to `<your-org>/Agentic_SOC` to restore clones. Discord/case open still succeeded.
